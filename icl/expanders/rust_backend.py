"""Rust backend scaffold emitter for ICL Intent Graph."""
...
class RustBackend(BackendEmitter):
    """Expands Intent Graph into Rust-like source.

    This backend is intentionally conservative and focuses on readable scaffold
    generation for future typed lowering passes.
    """
    ...
    def emit_module(self, graph: IntentGraph, context: ExpansionContext) -> str:
        ...
        if main_ids:
            for stmt_id in main_ids:
                lines.extend(self._emit_stmt(graph, stmt_id, indent=1))
        else:
    ...
    def _emit_function(self, graph: IntentGraph, node_id: str, indent: int) -> list[str]:
        ...
        param_src = ", ".join(f"{param['name']}: f64" for param in params)
        lines = [self.indent(f"fn {node.attrs['name']}({param_src}) -> f64 {{", indent)]

        if node.attrs.get("expr_body"):
            expr_id = graph.child_ids(node_id, "return_expr")[0]
            lines.append(self.indent(self._emit_expr(graph, expr_id), indent + 1))
        ...
        if body_ids:
            ...
            lines.append(self.indent("0.0", indent + 1))
        lines.append(self.indent("}", indent))
        return lines
    ...
    def _emit_stmt(self, graph: IntentGraph, node_id: str, indent: int) -> list[str]:
        ...
        if kind == "AssignmentIntent":
            ...
            return [self.indent(f"let mut {name} = {value_src};", indent)]
        ...
        if kind == "ExpressionIntent":
            ...
            if expr_node.kind == "CallIntent" and expr_node.attrs.get("callee_name") == "print":
                args = [self._emit_expr(graph, arg_id) for arg_id in graph.child_ids(expr_id, "arg")]
                arg = args[0] if args else "\"\""
                return [self.indent(f"println!(\"{{:?}}\", {arg});", indent)]
        ...
        if kind == "LoopIntent":
            ...
            lines = [self.indent(f"for {it} in ({start_src} as i64)..({end_src} as i64) {{", indent)]
        ...
        if kind == "FuncIntent":
            return self._emit_function(graph, node_id, indent)
        ...
        if kind == "ReturnIntent":
            ...
            return [self.indent("return 0.0;", indent)]
    ...
    def _emit_expr(self, graph: IntentGraph, node_id: str) -> str:
        ...
        if kind == "OperationIntent":
            ...
            return f"({operands[0]} {rust_operator} {operands[1]})"
